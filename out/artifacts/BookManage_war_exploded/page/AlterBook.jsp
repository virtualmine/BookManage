...
<%List<Books> books=(ArrayList)request.getAttribute("bookslist");%>
  ...
  <%for(int i = 0; i < books.size(); i++){%>
  		<button class="layui-btn btl"  onclick="location.href='searchalter.da?AlterBookName=<%=books.get(i).getBookName() %>'" type="button">
  		<div class="title_type">
        <%out.println(books.get(i).getBookName());%>
        </div>
        <%out.println(books.get(i).getAuthor()+" 著"+"</br>");%>
        
        <i class="layui-icon" style="font-size: 24px; color: #FFB800;">&#xe642;</i>  
        </button>
  <%}
 %>
 </div>
